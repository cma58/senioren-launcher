# 📱 Senior Launcher (0.8.3 Beta)

**The honest, open-source Android launcher for our elders. Created to make technology accessible, safe, and human again.**

[![License: GPL-3.0](https://img.shields.io/badge/License-GPL%20v3-blue.svg)](LICENSE)
[![Privacy](https://img.shields.io/badge/Privacy-100%25-brightgreen.svg)](PRIVACY.md)
[![Status: Beta](https://img.shields.io/badge/Status-0.8.3%20Beta-orange.svg)](#)

> **⚠️ Beta Stage:** This project is built in my spare time to give our (grand)parents their digital freedom back. It is currently in beta, which means feedback and help with testing are very welcome!

---

## 🛡️ Security & Privacy (EU & Android 16 Ready)
This project is updated to comply with the latest European security standards (**GDPR/AVG**) and is fully optimized for **Android 16 (Baklava)**:
- **Privacy by Design:** Explicit privacy labels for sensitive data (Contacts, GPS).
- **Secure Updates:** Update delivery via encrypted HTTPS connections.
- **Modern Display:** Full Edge-to-Edge support for Android 15/16.
- **100% Local:** Your data never leaves your device. No cloud, no tracking.

---

## 🇳🇱 Nederlandse versie (Dutch)
Zie de **[README_NL.md](README_NL.md)** voor de Nederlandstalige versie.

---

## 📸 Screenshots
<p align="center">
  <img src="screenshots/screenshot_home.jpeg" width="250" title="Home Screen">
  <img src="screenshots/screenshot_phone.jpeg" width="250" title="Phone">
  <img src="screenshots/screenshot_weather.jpeg" width="250" title="Weather & Advice">
</p>
<p align="center">
  <img src="screenshots/screenshot_medication.jpeg" width="250" title="Medication">
  <img src="screenshots/screenshot_emergency.jpeg" width="250" title="Emergency Info">
  <img src="screenshots/screenshot_remote_support.jpeg" width="250" title="Remote Support">
</p>

---

## 🌟 My Vision: "Senior-First"
Most smartphones are designed for young people. We turn that around. The Senior Launcher is built following strict UX rules for the elderly:
- **No Keyboards:** Everything works with large Plus/Minus buttons and simple lists.
- **Gigantic Elements:** Texts are at least 20-30sp. Buttons are at least 70dp high.
- **Digital Peace:** No unnecessary notifications or complicated swipe gestures.

## ❤️ Appreciation & Future
Senior Launcher is and will always remain free and open-source. I build this alongside my daily work because I believe digital freedom for seniors is a right, not a luxury.

**Want to help keep this project sustainable?**
- **Feedback:** Test the app and send me your findings.
- **Support:** A small contribution helps me cover server costs and acquire new test devices to ensure everything keeps working perfectly.

[![Support via PayPal](https://img.shields.io/badge/Support-PayPal-blue.svg?style=for-the-badge)](https://www.paypal.com/donate/?business=amine.chtaiti@gmail.com&no_recurring=0&currency_code=EUR)

---

## ✨ Key Features
- **📞 Calling:** Large dialer with photo favorites and 112 emergency confirmation.
- **💬 Messaging:** Extra-large SMS view with instant font size adjustment.
- **🆘 SOS:** Hold for 3 seconds for emergency calls and GPS location sharing.
- **💊 Medication:** Reminders with actual pill photos and stock tracking.
- **🎮 Remote Support:** Manage the phone via simple SMS codes (no internet required).

---

## 🏗️ Technology
- **Kotlin 2.1 & Jetpack Compose**
- **Android 16 SDK 36 Compliant**
- **Material 3 Design**

*"Technology should connect people, not exclude them."* ❤️
